...
const PdfPage = () => {
  ...
  const sendEmail = async (e, selectedPdf) => {
    e.preventDefault();
    try {
      await axios.post(
  ...
  return (
    <div className="pdfs">
      <Navbar />
      {showForms ? (
        <form onSubmit={(e) => sendEmail(e, selectedPdf)}>
          <input
            type="email"
